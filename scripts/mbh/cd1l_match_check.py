...
PATH = os.environ.get("MOJITO_ROOT",
    "/data/nbody/majoburo/.mojito_cache/brickmarket/mojito_light_v1_0_0/")
MBHB_L1 = os.path.join(PATH, "data", "MBHB", "L1")
BACKEND = os.environ.get("MBH_BACKEND", "cuda12x")
SENS_MODEL = "scirdv1"
...
MBH_HIGHER_MODES = tuple(int(m) for m in os.environ.get("MBH_HIGHER_MODES", "21,33,44").split(",") if m)
MBH_PHENOM_TOL = 1e-12
MBH_START_FREQ = float(os.environ.get("MBH_START_FREQ", "7e-5"))
MBH_RESPONSE_ORDER = int(os.environ.get("MBH_RESPONSE_ORDER", "30"))
...
# Persistent: the full-stream read is once-per-source and /tmp is ephemeral
# on compute nodes, so a /tmp cache would be re-read on every job.
CACHE_DIR = os.environ.get("CD1L_CACHE", "/data/nbody/majoburo/cd1l_pe/cache")
os.makedirs(CACHE_DIR, exist_ok=True)
DATA_CACHE = os.path.join(CACHE_DIR, f"mbh_mojito_data_id{MBHB_ID}.npz")
...
# and are regenerated.
TMPL_CACHE = os.path.join(CACHE_DIR, "mbh_mojito_tmpl_id{id}_{tag}.npz")
...
def template_td_cached(tag, sampling_params, window_t0, wdm_full):
    ...
    key = dict(n_win=int(N_WIN), dt=float(DT), window_t0=float(window_t0),
               mbhb_id=int(MBHB_ID), start_freq=float(MBH_START_FREQ),
               order=int(MBH_RESPONSE_ORDER), modes=str(MBH_HIGHER_MODES))
    if os.path.exists(p):
        try:
            with np.load(p, allow_pickle=True) as zc:
                ok = (int(zc["n_win"]) == key["n_win"]
                      ...
                      and "start_freq" in zc.files
                      and abs(float(zc["start_freq"]) - key["start_freq"]) < 1e-12
                      and int(zc["order"]) == key["order"]
                      and str(zc["modes"]) == key["modes"])
                if ok:
...
# ----------------------------------------------------------------------------
def _host(arr):
    """Bring ``arr`` back to the host.

    cupy refuses implicit np.asarray ("Please use .get()"), and the template
    cache is an .npz, which is host-only. Templates are returned to callers as
    host arrays and moved back per-transform by :func:`_on`.
    """
    get = getattr(arr, "get", None)
    return get() if get is not None else np.asarray(arr)


def _on(settings, arr):
    """Move ``arr`` onto ``settings``' array module.

    The cached window is numpy (np.savez), but with force_backend="cuda12x" the
    domain transform calls cupy's rfft, which raises
    "The input array a must be a cupy.ndarray" on a host array. Templates built
    on the GPU are already cupy, so asarray is a no-op there.
    """
    return settings.xp.asarray(arr)
...
def fd_setup(data_td, window_t0, win):
    ...
    data_fd = TDSignal(_on(td_set, data_td), td_set).transform(
        fd_set, window=_on(td_set, win))
...
def fd_signal(td, td_set, fd_set, win):
    return TDSignal(_on(td_set, td), td_set).transform(
        fd_set, window=_on(td_set, win))
...
def time_scan(ac, base_sig, fd_set, taus):
    ...
    f = fd_set.xp.asarray(fd_set.f_arr)
    base = base_sig.arr.copy()
    best = (-1.0, 0.0)
    for tau in taus:
...
def _wdm_report(ws, data_td, template, td_set, win, tag):
    d = TDSignal(_on(td_set, data_td), td_set).transform(
        ws, window=_on(td_set, win))
    t = (template if not isinstance(template, np.ndarray)
         else TDSignal(_on(td_set, template), td_set).transform(
             ws, window=_on(td_set, win)))
...
def make_plots(data_td, tmpl_td, td_set, fd_set, win, window_t0, abs_merger):
    f = _host(fd_set.f_arr)
    d_fd = _host(fd_signal(data_td, td_set, fd_set, win).arr)
    t_fd = _host(fd_signal(tmpl_td, td_set, fd_set, win).arr)
